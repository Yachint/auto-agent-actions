...
- The first Landlock preflight correctly denied the `/tmp` write, but Debian `dash` exited with status 2 when redirection failed on the `:` special builtin before the same shell's conditional could handle it. The write attempt now runs in a child shell, allowing the parent to treat the expected denial as success while still failing closed if the write unexpectedly succeeds.
