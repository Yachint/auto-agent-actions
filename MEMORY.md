...
- Added `docs/ACCEPTANCE.md` mapping every first-release criterion to local evidence and listing the remaining live Docker, GitHub, stale-head, recovery, credential-isolation, and backup checks. The current local build passes with 109 standard tests; the opt-in real broker socket test also passed, Compose security boundaries were machine-checked, and the production npm audit reported zero vulnerabilities.
